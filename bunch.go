...
func checkOptions() {
	if options.Prefix == "" {
		fatal("Please specify --prefix")
	}
	if options.Manifest == "" {
		fatal("Please specify --manifest")
	}
	if options.Path == "" {
		fatal("Please specify --path")
	}
	if options.S3Key == "" {
		fatal("Please specify --s3-key")
	}
	if options.S3Secret == "" {
		fatal("Please specify --s3-secret")
	}
	if options.S3Bucket == "" {
		fatal("Please specify --s3-bucket")
	}
}
